...
// --- Schema property allowlists (from schemas/*.schema.json) ---
...
var schemaAllowedFields = map[string]map[string]bool{
	...
	"kit": {
		...
		"stat_bonuses": true, "equipment": true,
		"content": true, "metadata": true,
	},
	"perk": {
		"name": true, "type": true, "prerequisites": true,
		"content": true, "metadata": true,
	},
	"career": {
		"name": true, "type": true, "skill": true, "language": true,
		"renown": true, "wealth": true, "project_points": true, "perk": true,
		"content": true, "metadata": true,
	},
	"ancestry": {
		"name": true, "type": true, "signature_trait": true,
		"content": true, "metadata": true,
	},
	"culture": {
		"name": true, "type": true, "environment": true,
		"organization": true, "upbringing": true, "skill": true, "language": true,
		"content": true, "metadata": true,
	},
	"title": {
	...
	tests := []struct {
		...
		},
		{
			...
			map[string]any{
				...
				"stat_bonuses": map[string]string{"stamina": "+3"},
				"equipment":    []string{"Light armor", "Dagger"},
			},
			"Kit description.",
		},
		{
			"perk with prerequisites",
			map[string]any{"name": "Coward", "type": "perk", "prerequisites": "None"},
		...
		{
			"ancestry with signature_trait",
			map[string]any{"name": "Hakaan", "type": "ancestry", "signature_trait": "Mighty"},
			"Ancestry body.",
		},
		{
			...
			map[string]any{
				...
				"upbringing": "Martial", "skill": "Nature", "language": "Caelian",
			},
			"Culture body.",
		},
		{
			"title with echelon and benefits",
			map[string]any{
		...
		},
		{
			...
			map[string]any{"name": "Criminal Past", "type": "complication"},
...
func TestSchema_FieldTypes_Kit(t *testing.T) {
	parsed := &content.ParsedContent{
		Frontmatter: map[string]any{
			...
			"equipment": []string{"Heavy armor", "Shield", "Weapon"},
		},
		Body: "Kit body with stat table.",
	}
	out := TransformToSDKFormat("", parsed)

	// stat_bonuses should be an object
	if _, ok := out["stat_bonuses"].(map[string]string); !ok {
		t.Errorf("stat_bonuses should be map[string]string, got %T", out["stat_bonuses"])
	}

	// equipment should be an array
	if _, ok := out["equipment"].([]string); !ok {
		t.Errorf("equipment should be []string, got %T", out["equipment"])
	}
...
func TestSchema_FieldTypes_Treasure(t *testing.T) {
	...
	for _, field := range []string{"treasure_type", "level", "rarity"} {
...
func TestSchema_FieldTypes_Career(t *testing.T) {
	...
	for _, field := range []string{"skill", "language", "renown", "wealth", "project_points", "perk"} {
		if _, ok := out[field].(string); !ok {
			t.Errorf("%s should be string, got %T", field, out[field])
		}
	}
...
func TestSchema_FieldTypes_Culture(t *testing.T) {
	...
	for _, field := range []string{"environment", "organization", "upbringing", "skill", "language"} {
		if _, ok := out[field].(string); !ok {
			t.Errorf("%s should be string, got %T", field, out[field])
		}
	}
...
func TestSchema_JSONRoundtrip(t *testing.T) {
	tests := []struct {
		...
		{"career", map[string]any{"name": "Sage", "type": "career", "skill": "Lore"}, "Body."},
...
func TestSchema_PassthroughNoFeatureFields(t *testing.T) {
	...
	featureOnlyFields := []string{"feature_type", "effects", "usage", "ability_type", "trigger", "flavor"}
