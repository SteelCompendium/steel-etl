...
| `internal/site/cards.go` | Rich `.sc-card` index cards for Browse-tab type indexes (kit, class, ancestry, …, plus `rule` glossary-term leaves under `rule/<group>/`, labelled by their topic group) + shared `card()`/`crestSVG`/`iconPaths` |
...
(`SkillParser`, added 2026-06-08). Each skill group also has a self-named-leaf
landing page `skill.<group>/<group>` emitted by the **`skill-group`** container
parser (`internal/content/skill_group.go`) — the same self-named pattern as the
`monster-group` container (`monster.<category>/<category>`) — so prose can link
to "the <group> skill group". The container pushes no path context; child skills
get their group from their own `@group`. Site-side, the `skill/` Browse root
renders `.sc-folder` group cards (`usesFolderIndex`) and each group dir renders a
`.sc-card` skill grid (`buildCardsContent`, container page excluded via
`dropSelfNamed`).
