...
- Fixture statblocks' 2-column stat grid + italic role line are parsed (`applyFixtureGrid`, `monster.go`) into `stamina`/`size` + `role`/`terrain_type` + `statblock_kind: fixture` (resolved the former gap, was workspace `FOLLOWUPS.md` #6). Featureblocks/terrain now emit structured `kind`/`level`/`flavor`/`stats[]`/`features[]` validated by `featureblock.schema.json` (both copies); build-time site rendering via `internal/site/featureblock_page.go` (Plan 2 — featureblock/dynamic-terrain scope); fixture routing ships in Plan 3 (`internal/site/fixture_page.go` adapter + `.fb__band--adv` advancement-band rendering); retainer-advancement split ships in Plan 4 (`internal/site/retainer_page.go` — splits the body at the `**Level N Retainer Advancement Ability**` bold labels, keeps the island base-only, appends an "Advancement Abilities" card). **Plan 5** restructures companions into `monster.companion.beastheart.*` (5a, shipped) and mints embeddable `monster.companion.beastheart.advancement-features/<species>` featureblock entities (5b, shipped — `FeatureblockParser` companion branch in `monster.go` embeds the Level-3/6/10 child features via `collectChildFeatures`, renders a Forged Band card on the standalone page; see `docs/scc-log.md`); fixture parity `monster.fixture.<element>.featureblock` is 5c; **Plan 6** = retainer rework. Spec: `docs/superpowers/specs/2026-06-13-companion-restructure-advancement-featureblocks-design.md`. See [`docs/statblocks.md`](docs/statblocks.md).
