...
⚠️ **The `codes` array in `classification.json` accumulates — it is not pruned.** A
plain `gen` *adds* newly-seen codes but leaves orphaned entries for codes no longer
emitted (e.g. after a rename/removal migration), so the registry can contain stale
codes the live site never references (the site + permalink stubs are built from page
frontmatter, not this list, so the deployed site is unaffected). To get a registry
that reflects only live codes — e.g. when verifying a migration that *removes*
codes — rebuild fresh: `rm classification.json && gen --all`. Safe because the file
is gitignored/regenerated.
