...
- Fixture statblocks' 2-column stat grid + italic role line are parsed (`applyFixtureGrid`, `monster.go`) into `stamina`/`size` + `role`/`terrain_type` + `statblock_kind: fixture` (resolved the former gap, was workspace `FOLLOWUPS.md` #6). Featureblocks/terrain now emit structured `kind`/`level`/`flavor`/`stats[]`/`features[]` validated by `featureblock.schema.json` (both copies); build-time site rendering via `internal/site/featureblock_page.go` (Plan 2 — featureblock/dynamic-terrain scope); fixture routing ships in Plan 3 (`internal/site/fixture_page.go` adapter + `.fb__band--adv` advancement-band rendering); Plans 4–5 (retainer-advancement split, companion cards) remain; see [`docs/statblocks.md`](docs/statblocks.md).
