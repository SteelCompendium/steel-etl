...
**Search exclusion (SC-306).** Every spliced copy is wrapped in
`<address class="sc-embed" data-search-exclude="">` (`markSearchExcluded`) —
Material's search parser (`plugins/search/plugin.py`) keys excluded elements by
TAG NAME, so the wrapper must be a tag that never otherwise occurs inside a
card — and loses its nested `sc-feat-` ids (`stripFeatIDs`), so the container
page's search entry only ever indexes the leaf's own canonical page. The Browse
kit index tile's signature-ability splice (`kitSignatureCardHTML`, `cards.go`)
gets the same treatment even though it bypasses `embedItemCards` entirely.
