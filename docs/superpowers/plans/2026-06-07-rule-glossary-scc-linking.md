...
> **Execution status (2026-06-07):** Phases 0–5 + 7 **DONE and DEPLOYED LIVE**. Shipped: `rule.<group>/<term>` type (108 codes, 12 groups), all rule sections annotated (gen 0 WARN, 1915 classified), Browse publishing, reference-table + linking-guide, and the full **Introduction glossary** linked (167 headwords; heroes ~4,789 SCC links). Deployed via `just deploy` (API → org repo `6d0244d`; v2 site `d590570`) + data repos pushed (`data-rules 98a9de9`, `data-unified 3e48018`). steel-etl `c7e1c78`, workspace `f3bb570`. **Phase 6 (full in-prose sweep across the document body) is DONE & DEPLOYED LIVE (2026-06-08)** — tracked as workspace `FOLLOWUPS.md` #9 (now marked complete). Mid-execution recovery note: 3 rule annotations were mistakenly stacked above non-code `feature-group` containers (Main/Maneuver/Move Actions); fixed by anchoring `Turn` on `### Taking a Turn` and reusing `rule.combat/turn` for those three.
>
> **Phase 6 complete (2026-06-08):** the whole document body was swept across ~5 batches — health/dice/combat/character/resource/test/downtime/treasure/world/general/negotiation groups, plus the combat keywords and common words. **109 `rule.*` codes** (the original 108 + `test-difficulty`, minted 2026-06-08 to close a Phase-3 annotation gap; any "108" elsewhere predates it). Heroes doc went **4,789 → ~17,040** SCC links (12,386 `rule.*`). Linking-density decisions (recorded in memory `comprehensive-linking-density.md`): **comprehensive** for most mechanic terms; **conservative** (defining-sentence anchors only, no statblock effect lines) for the ubiquitous `damage`/`creature`/`ability`/`target`/`ally`/`enemy`; `side`/`line`/`wall`/`ground` deliberately left (mundane-dominated). **Footguns encountered & fixed:** (1) literal `[Term]` ability-notation placeholders (`[potency value]`, `[damage type]`) → bare-word linking nests into `[[…]]`; the broadened malformed grep `\]\(scc:[^)]*\)\]` (filter `)\]\(scc:`) catches it. (2) Multi-word terms linked before single-word components. (3) Verify a target code exists (`jq -r '.codes[]' classification.json`) before linking — `test-difficulty` was unminted. Deployed live across 5 deploys; all repos in sync.
