...
> **Execution status (2026-06-07):** Phases 0–5 + 7 **DONE and DEPLOYED LIVE**. Shipped: `rule.<group>/<term>` type (108 codes, 12 groups), all rule sections annotated (gen 0 WARN, 1915 classified), Browse publishing, reference-table + linking-guide, and the full **Introduction glossary** linked (167 headwords; heroes ~4,789 SCC links). Deployed via `just deploy` (API → org repo `6d0244d`; v2 site `d590570`) + data repos pushed (`data-rules 98a9de9`, `data-unified 3e48018`). steel-etl `c7e1c78`, workspace `f3bb570`. **Phase 6 (full in-prose sweep across the document body) is IN PROGRESS — tracked as workspace `FOLLOWUPS.md` #9** (multi-session). Mid-execution recovery note: 3 rule annotations were mistakenly stacked above non-code `feature-group` containers (Main/Maneuver/Move Actions); fixed by anchoring `Turn` on `### Taking a Turn` and reusing `rule.combat/turn` for those three.
>
> **Phase 6 progress (2026-06-08, steel-etl `6fc89ba`..`7f5fe07`, local/unpushed):** First high-yield batch done — winded, dying, flanking, concealment (all mechanic), cover (mechanic-only, 22/43), and the ultra-high-frequency core terms stamina, temporary-stamina, recoveries/recovery-value, surge, hero-token, heroic-resource linked **comprehensively** (Scott's call — every mechanic instance, not first-per-section). `suffocating` was already fully linked. **Next:** the characteristics (Might/Agility/Reason/Intuition/Presence + `characteristic`) — case-sensitive on capitalized proper-noun forms to avoid the modal "might"/common "reason"/"presence"; then the common-word tail (edge, bane, size, distance, strike, damage). **Footguns:** (1) literal `[Term]` ability-notation placeholders → bare-word linking makes `[[…]]`; broaden the malformed grep with `\]\(scc:[^)]*\)\]` (filter `)\]\(scc:`). (2) Link multi-word terms before their single-word components. Stale plan line numbers — the doc grew; re-find anchors with `grep -nE '^#{3,5} <Heading>$'`.
